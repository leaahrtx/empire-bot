...
/**
 * Arguments présents sur tous les appels.
 *
 * `--js-runtimes node` est indispensable : depuis 2026, YouTube impose des
 * défis JavaScript que yt-dlp ne sait résoudre qu'avec un moteur externe. Seul
 * deno est activé par défaut et n'est pas installé ici, alors que Node l'est.
 *
 * Sans ce moteur, l'extraction est dégradée et YouTube répond bien plus souvent
 * « Sign in to confirm you're not a bot ».
 */
const ARGS_COMMUNS = ['--ignore-config', '--no-warnings', '--js-runtimes', 'node'];
...
/**
 * bin/ est exclu du dépôt (36 Mo) : après un déploiement, le binaire doit être
 * réinstallé. Sans ce contrôle, l'erreur remontée serait un ENOENT illisible.
 */
let ytdlpVerifie = false;
function verifierYtdlp() {
  if (ytdlpVerifie) return;
  try {
    accessSync(YTDLP, constants.X_OK);
    ytdlpVerifie = true;
  } catch {
    throw new Error(
      'binaire yt-dlp absent ou non exécutable. Lance `npm run ytdlp` dans la console du serveur.',
    );
  }
}
