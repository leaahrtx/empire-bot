...
function resoudreRole(guild) {
  ...
  return (
    guild.roles.cache.get(cle) ??
    guild.roles.cache.find((r) => r.name === cle) ??
    // Tolérance sur la casse : une majuscule de travers dans la configuration
    // ferait autrement échouer l'attribution sans erreur visible.
    guild.roles.cache.find((r) => r.name.toLowerCase() === cle.toLowerCase()) ??
    null
  );
}
...
/**
 * Remplit le gabarit de config.anniversaires.message. Le rôle est mentionné à
 * partir de l'objet résolu : son identifiant n'a pas à être connu à l'avance.
 */
function composerAnnonce(fetes, role, anneeCourante) {
  const membres = fetes.map(([id]) => `<@${id}>`).join(', ');
  ...
  return config.anniversaires.message
    .replaceAll('{membre}', membres)
    .replaceAll('{role}', role ? `<@&${role.id}>` : '')
    .replaceAll('{age}', ages)
    .replace(/[ \t]+/g, ' ')
    .trim();
}
...
export async function verifierAnniversaires(client, guildId = null) {
  ...
  for (const guild of client.guilds.cache.values()) {
    ...
    if (salon && fetes.length > 0) {
      const contenu = composerAnnonce(fetes, role, anneeCourante);

      if (
        contenu.includes('@everyone') &&
        !guild.members.me?.permissions.has(PermissionFlagsBits.MentionEveryone)
      ) {
        console.warn(
          `[anniversaires] Permission « Mentionner @everyone » manquante sur ${guild.name} : ` +
            'le message partira sans notifier le serveur.',
        );
      }
      ...
      await salon
        .send({
          content: contenu,
          // Sans cette autorisation explicite, Discord affiche @everyone en
          // texte brut sans notifier personne.
