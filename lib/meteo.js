...
async function diffuser(client, embed) {
...
export async function annoncerMeteoJour(client) {
  const releves = await releverVilles(1);
  if (releves.length === 0) {
    console.error('[meteo] Aucune ville exploitable, annonce quotidienne annulée.');
    return;
  }
  await diffuser(client, construireEmbedJour(releves));
}

/** Prévisions à sept jours pour toutes les villes configurées. */
export async function annoncerMeteoSemaine(client) {
  const releves = await releverVilles(7);
  if (releves.length === 0) {
    console.error('[meteo] Aucune ville exploitable, annonce hebdomadaire annulée.');
    return;
  }
  await diffuser(client, construireEmbedSemaine(releves));
}
