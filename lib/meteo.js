...
/** Lundi et dimanche d'une semaine ISO, au format AAAA-MM-JJ. */
export function bornesSemaineISO(annee, semaine) {
  // Le 4 janvier appartient toujours à la semaine ISO 1, quelle que soit l'année.
  const quatreJanvier = new Date(Date.UTC(annee, 0, 4));
  const decalage = (quatreJanvier.getUTCDay() + 6) % 7; // 0 = lundi
  const lundiSemaine1 = quatreJanvier.getTime() - decalage * 86_400_000;
  const lundi = new Date(lundiSemaine1 + (semaine - 1) * 7 * 86_400_000);
...
/** Prévisions sur un intervalle de dates précis plutôt qu'à partir d'aujourd'hui. */
export function previsionsIntervalle(lieu, debut, fin) {
  return interrogerPrevisions(lieu, (url) => {
    url.searchParams.set('start_date', debut);
    url.searchParams.set('end_date', fin);
  });
}

export function previsions(lieu, jours = 7) {
  return interrogerPrevisions(lieu, (url) => {
    url.searchParams.set('forecast_days', String(jours));
  });
}

async function interrogerPrevisions(lieu, appliquerPeriode) {
...
/** Date du jour au format AAAA-MM-JJ, vue depuis le fuseau du lieu. */
function dateDuJour(timezone) {
...
export function construireEmbed(lieu, jours, titre = null) {
  // On compare la date réelle plutôt que la position : sur une période future,
  // le premier jour n'est pas « aujourd'hui ».
  const ceJour = dateDuJour(lieu.timezone);

  const lignes = jours.map((j) => {
    const [emoji, description] = decrire(j.code);
    const etiquette = j.date === ceJour ? "Aujourd'hui" : nomJour(j.date);
    const pluie =
